...
- protected routes are not enforced until the shared auth UI and Ory backend exist

Environment:
```bash
# Set the legacy flags to false when enabling the shared auth UI and Ory backend.
...
2. protected app routes require login when the shared auth UI and Ory backend are enabled
...
- `/processing-page/*` needs shared auth + Ory + Stripe + Stripe products
