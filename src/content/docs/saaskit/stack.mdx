...
- Shared ProChat auth UI backed by Ory, Stripe (billing), Resend (email), WordPress (blog), and n8n (automation) are all optional features that plug into the stack when you opt in. Each integration keeps SaaSKit’s router and API handlers running safely when keys are missing. See `docs/public/integrations.md` for the configuration checks and graceful fallbacks.
