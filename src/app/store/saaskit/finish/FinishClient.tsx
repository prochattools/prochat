...
export default function FinishClient() {
  ...
  async function submitClaim(payload: { session_id?: string; email?: string; github_username: string }) {
    ...
    try {
      const response = await fetch("/api/store/saaskit/claim", {
        method: "POST",
      ...
      console.error("[store/saaskit/finish] Claim request failed", error);
    ...
    <div className="mx-auto max-w-3xl px-4 py-16">
      ...
      {showFallback && (
        <div className="mt-6 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-700 dark:bg-[#0B111B]">
          ...
          <p className="mt-2 text-sm text-slate-600 dark:text-slate-300">
            Use the email you entered during Stripe checkout. We will find your latest paid SaaSkit purchase and link it.
          </p>
          <div className="mt-4 space-y-3">
            <label className="block text-sm font-medium text-slate-700 dark:text-slate-200">
              Checkout email
