...
export async function handleKitClaim(req: Request, productSlug: ProductSlug) {
	try {
		...
		if (sessionId) {
			const status = await getSessionStatusById(sessionId, productSlug)
		...
		if ('ok' in provisionResult) {
			...
			return NextResponse.json({
				...
				githubUsername,
				accessState: provisionResult.accessState,
			})
		}
		...
		if (provisionResult.error === 'forbidden') {
			console.error(`[store/${productSlug}/claim] GitHub permission error`, {
				sessionId: targetSessionId,
				productSlug,
				githubUsername,
		...
		console.error(`[store/${productSlug}/claim] Unknown GitHub API error`, {
			sessionId: targetSessionId,
			details: provisionResult.message,
		})
		return NextResponse.json(
		...
		console.error(`[store/${productSlug}/claim] Unexpected error`, error)
		return NextResponse.json(
			{
				error: 'server_error',
				message:
