...
export const BlueprintCard: React.FC<BlueprintCardProps> = ({ children, className = '', label, onClick }) => (
  <div
    ...
    className={`
      ...
      dark:border-[#1E242D]
      dark:shadow-none
      dark:hover:border-[#2A3445]
      dark:hover:shadow-[0_12px_30px_-12px_rgba(29,78,216,0.25)]
      ${className}
    `}
  >
    {/* Label */}
    {label && (
      <div className="absolute top-4 right-5 text-[9px] font-mono text-slate-400 uppercase tracking-wider group-hover:text-[#1D4ED8] transition-colors z-20 pointer-events-none dark:text-slate-500 dark:group-hover:text-[#2563EB]">
        {label}
      </div>
    )}
