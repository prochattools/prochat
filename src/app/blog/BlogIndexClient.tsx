...
const systemFoundationSlugs = new Set([
...
export default function BlogIndexClient({ posts }: BlogIndexClientProps) {
  ...
  const systemFoundationPosts = useMemo(
    () =>
      startHerePosts
        .filter(post => systemFoundationSlugs.has(post.slug))
        .sort(sortPillarPosts),
    [startHerePosts],
  )

  const startHerePost =
    startHerePosts.find(post => post.slug === canonicalStartHereSlug) ||
    startHerePosts.find(post => post.pillar) ||
    startHerePosts[0] ||
    null

  const groupedPosts = useMemo(() => {
    const excludedSlugs = new Set([
      ...(startHerePost ? [startHerePost.slug] : []),
      ...systemFoundationPosts.map(post => post.slug),
    ])

    const nonFeatured = filteredPosts.filter(post => !excludedSlugs.has(post.slug))
  ...
  }, [filteredPosts, startHerePost, systemFoundationPosts])

  return (
    <div className="mt-6">
      {systemFoundationPosts.length > 0 && (
        <section>
          <div className="max-w-2xl">
            ...
            <p className="mt-2 text-sm leading-6 text-muted-foreground">
              High-level context for how ProChat structures SaaS execution.
            </p>
          </div>

          <div className="mt-4 grid gap-4 md:grid-cols-2">
            {systemFoundationPosts.map(post => (
              <article key={post.slug} className="h-full">
                ...
                >
                  <Panel
                    tone="soft"
                    padding="compact"
                    className="flex h-full flex-col border-border-subtle/75 bg-surface-soft/72 shadow-surface transition-all duration-200 ease-out group-hover:border-border-strong/70 group-hover:bg-surface group-hover:shadow-elevated"
                  >
                    ...
                    <h3 className="mt-3 font-brand text-xl font-bold leading-snug text-foreground transition-colors duration-200 group-hover:text-primary md:text-2xl">
                      {post.title}
                    </h3>
                    <p className="mt-3 flex-1 text-sm leading-6 text-muted-foreground">
                      {post.description}
                    </p>
                    <div className="mt-5 border-t border-border-subtle/70 pt-4 text-xs font-medium text-foreground underline decoration-border-strong/80 underline-offset-4 transition-all duration-200 group-hover:decoration-foreground/70">
                      Read
      ...
      <section className="mt-10">
        <div className="max-w-2xl">
          ...
          <p className="mt-3 text-sm leading-6 text-muted-foreground">
            Then follow the structured path below.
          </p>
        </div>
