...
export const BlueprintCard: React.FC<BlueprintCardProps> = ({ children, className = '', label, onClick }) => (
  <div 
    ...
    className={`
      ...
      dark:hover:shadow-[0_12px_30px_-12px_rgba(91,73,245,0.25)]
      ${className}
    `}
  >
    {/* Label */}
    {label && (
      <div className="absolute top-4 right-5 text-[9px] font-mono text-slate-400 uppercase tracking-wider group-hover:text-[#5b49f5] transition-colors z-20 pointer-events-none dark:text-slate-500 dark:group-hover:text-[#9b8bff]">
        {label}
      </div>
    )}
