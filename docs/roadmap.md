...
deployment_observation_source: Main workflow run 31012683840 (PXF-018A security remediation); all 8 canonical routes HTTP 200; direct production /api/version matches full deployed SHA; built 2026-08-05T14:03:48Z
...
current_program_state: PXF-018A security remediation is deployed to production (SHA 38bc212). All eight canonical routes serve (/, /memory, /memory-qa, /workbench, /docs, /contact, /privacy, /terms all HTTP 200). MailerLite credential exposure removed (hardcoded fallback eliminated; now requires MAILERLITE_API_KEY env var only). Project enumeration endpoint hardened (GET /api/tenants/projects returns 501 fail-closed). CI wired with deterministic secret scanning and comprehensive API security suite: API protection 12/12 tests (tenants/projects 501 + no-Prisma, contact honeypot, waitlist honeypot, preferences token-gating, stripe missing/invalid signature, 5 fail-closed routes); fixed-window rate-limiter 4/4 tests; combined test:security-api 16/16 tests; secret-source policy 7/7 tests. Authentication boundary audit corrected: /memory, /memory-qa, /workbench confirmed as public discovery pages (not protected session failures). Owner decision register reduced from 23 to 22 items. All checks passed (TypeScript ✓, ESLint ✓, docs validation ✓, build ✓ 109 pages, browser evidence ✓, API security tests ✓). External MailerLite credential rotation PENDING. Phase 11 PARTIAL (inventory created, cleanup deployed, auth audit corrected). Phase 13 continuous governance active.
...
deployment_status: VERIFIED — production /api/version matched 38bc212ad315d211812f4c4432b76afbf66a63e0 after PXF-018A deployment (security remediation: MailerLite credential removal, project API fail-closed, comprehensive API security tests, CI secret scanning); all 8 canonical routes HTTP 200; Image: ghcr.io/prochattools/prochat:38bc212ad315d211812f4c4432b76afbf66a63e0; built 2026-08-05T14:03:48Z; CI: TypeScript ✓, ESLint ✓, secret-sources scan ✓, build ✓ 109 pages, browser evidence ✓, test:security-api ✓ (16/16 tests passed, including 12 API protection + 4 rate-limiter unit tests), performance evidence collected (informational)
```
