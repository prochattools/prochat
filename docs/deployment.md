...
- Production is **hands-off**: provisioning, migrations, backups, and smoke checks are executed automatically by the runtime deploy gate during container start.
...
Set these in Dokploy env (values are typically generated by `npm run prokit-core:bootstrap -- <app-slug>` and copied from `.env.production`).

Required:

```bash
NODE_ENV=production
APP_SLUG=<app_slug>                  # repo-derived (repo folder name with hyphens removed)
...
Optional (destructive legacy cleanup):

```bash
# If set, the deploy gate will drop the legacy tenant after a successful deploy:
# - DROP SCHEMA tenant_<LEGACY_APP_SLUG> CASCADE
# - DROP ROLE tenant_<LEGACY_APP_SLUG>_user
# - DELETE FROM public.tenants WHERE slug=<LEGACY_APP_SLUG>
#
# This is DATA LOSS. It also disables the auto-rename behavior for LEGACY_APP_SLUG.
PROKIT_DROP_LEGACY_TENANT=1
```
...
- `APP_SLUG` must match `[a-z0-9_]+` and must equal the repo-derived slug (repo folder name with hyphens removed).
...
Do not exec into production to run provisioning/migrations manually.
